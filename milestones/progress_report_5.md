# Final Milestone Progress Report

## GitHub Project Setup
Project board created: YES <br />
Project board shared with codepathreview: YES<br />

## Issues Created
List the title of each issue you created, one per line:
- Access Control
- Custom Tags
- Filter Collections
- Browse Collections
- Sort Media
- Search Custom (User-Added) Media
- Add Custom Media
- Seed SQL Tables
- Create tables using SQL
- Create Media Card
- Link to Media
- Deploy to Render
- Auto-Generate Starter Tags for New Users

## GitHub Milestones Created
List the name of each GitHub Milestone you created, one per line:
- Milestone 3 - Unit 7
- Milestone 4 - Unit 8
- Final Milestone - Unit 9

## Final Milestone Completion
Replace the blank before the percent sign with a number from 0 to 100.<br />
Completion percentage for Final Milestone - Unit 9: 100%<br />
List each issue you completed this unit and the main file or folder where that work lives, one per line:
- Deploy to Render - server/server.js, server/package.json, server/.env.example, client/.env.example
- Auto-Generate Starter Tags for New Users - server/config/auth.js
- Access Control - server/server.js (cross-domain session cookie, trust proxy), live GitHub OAuth verified on the deployed site

## Features Completed This Unit
List each feature you completed and checked off in readme.md this unit, one per line:
- Search Pre-existing Media
